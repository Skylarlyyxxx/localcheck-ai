...
const scoreWeights = [
  ["Currency", "15%"], ["Payment", "20%"], ["Trust", "25%"],
  ["SEO", "15%"], ["Legal & Policy", "15%"], ["Language", "10%"],
];
...
export default function ReportPage() {
  ...
  return <main className="report-page"><div className="report-shell">
    ...
    <details className="scoring-details"><summary>How this score is calculated</summary><div className="scoring-details-content"><p>The overall score is a weighted combination of six deterministic, market-specific signal checks. A detected signal indicates public homepage evidence only; it does not confirm checkout availability or legal compliance.</p><div className="weight-grid">{scoreWeights.map(([name, weight]) => <div key={name}><span>{name}</span><strong>{weight}</strong></div>)}</div></div></details>
